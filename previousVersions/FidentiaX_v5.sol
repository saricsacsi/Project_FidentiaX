...
contract FidentiaXToken is MintableToken {
  ...
  address public fdXDeposit  = 0x006EC6E3F6225eb6E802C65532ceD362A04D382a;      // deposit address for findentiaX Seed Contributors

  uint256 public constant fdXSeed  = 35 * 10**6  * 10**18;



     // constructor
  function FidentiaXToken()
    {
      
      balances[fdXDeposit] = fdXSeed;                                         // Deposit fdX share
      
      totalSupply = fdXSeed;

      
      Transfer(0x0,fdXDeposit,fdXSeed);

     
    allowed[fdXDeposit][fdXDeposit] = fdXSeed;
    Approval(fdXDeposit, fdXDeposit, fdXSeed);
  ...
  */
  ...
  function approve(address _spender, uint256 _value) hasStartedTrading public returns (bool) {
...
contract FidentiaXTokenSale is Ownable {
 ...
 FidentiaXToken public token;
  ...
  uint256 public tokensForSale;  // 95 Million Tokens for SALE
  ...
  function FidentiaXTokenSale() public {
    ...
    startTimestamp = 1508684400; // change!!! 1509930000  Monday November 06, 2017 09:00:00 (am) in time zone Asia/Singapore (SGT)
  ...
  */
  ...
  function getCurrentTimestamp() internal returns (uint256) {
    ...
    function getRateAt(uint256 at) constant returns (uint256) {
        if (at < startTimestamp) {
            return 0;
        } else if (at >= startTimestamp && at < (tier1Timestamp)) {
            return 575;
        } else if (at >= tier1Timestamp && at < (tier2Timestamp)) {
            return 550;
        } else if (at >= tier2Timestamp && at < (endTimestamp)) {
            return 500;
        } else {
            return 0;
        }
    ...
    uint256 cap10k = 33 ether;
    uint256 cap50k = 165 ether;
    uint256 cap150k = 500 ether;
    uint256 cap300k = 1000 ether;
    
    function getBonusRate(uint256 rate, uint256 value) public constant returns (uint256) {
        if (value < cap10k)
            return rate * 100;
        if (value < cap50k)
            return rate * 105;
  ...
  * @dev set a new CS representative
  ...
  function buyTokens(address beneficiary, uint256 amount) onlyAuthorised internal {
    ...
    // Calculate token amount to be purchased
        ...
        uint256 bonusRateMul100 = getBonusRate(actualRate,amount);
        uint256 tokens = amount * bonusRateMul100 / 100;
